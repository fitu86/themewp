...
jQuery( document ).on( 'action.ready_carz', function() {
	...
	$document.on( 'action.resize_carz', function() {
		_list_products_height = $list_products.length > 0 ? $list_products.height() : 0;
	} );
	...
	// Change display mode
	//--------------------------------
	jQuery( '.woocommerce,.woocommerce-page' ).on( 'click', '.carz_shop_mode_buttons a', function(e) {
		var $self = jQuery(this),
			mode  = $self.hasClass( 'woocommerce_thumbs' ) ? 'thumbs' : 'list';
		carz_set_cookie( 'carz_shop_mode', mode, 365 * 24 * 60 * 60 * 1000 );   // 1 year
	...
	// Add buttons inc/dec
	function carz_woocommerce_add_inc_dec() {
			...
			if ( $self.find('>input[name="quantity"][type="hidden"]').length === 0 ) {
				...
				$self
					.on( 'click', '>span', function(e) {
						carz_woocomerce_inc_dec_click( jQuery( this ) );
						e.preventDefault();
						return false;
					} );
			} else {
	...
	function carz_woocomerce_inc_dec_click(button) {
	...
	$document.on( 'action.scroll_carz', function(e) {
		...
		if ( $list_products.length > 0 && $list_products.offset().top + _list_products_height < carz_window_scroll_top() + carz_window_height() * 1.5 ) {
	...
	//-------------------------------------------------------------------------
	if ( ! CARZ_STORAGE['select_container_disabled'] ) {
	...
	jQuery( document.body ).on( 'updated_wc_div', function() {
		...
		carz_create_observer( 'update_woocommerce_cart', table_cart.parents('form').eq(0), function( mutationsList ) {
			for ( var mutation of mutationsList ) {
				if ( mutation.type == 'childList' || mutation.type == 'subtree' ) {
					carz_remove_observer( 'update_woocommerce_cart' );
					table_cart = jQuery( 'table.cart' );
					carz_woocommerce_update_cart( 'update' );
					break;
				}
			}
		} );
	...
	if ( table_cart.length > 0 ) {
		carz_woocommerce_update_cart( 'init' );
	}

	// Resize action
	$window.on( 'resize', function() {
		if ( table_cart.length > 0 ) {
			carz_woocommerce_update_cart( 'resize' );
		}
	} );

	// Update cart display
	function carz_woocommerce_update_cart(status){
		setTimeout(
			function() {
				var w = window.innerWidth;
				if (w == undefined) {
					w = carz_window_width() + ( carz_window_height() < $document.height() || carz_window_scroll_top() > 0 ? 16 : 0 );
				}
				...
				if ( w >= CARZ_STORAGE['mobile_layout_width'] && status == 'resize' && jQuery( 'table.cart .mobile_cell' ).length > 0) {
					if ( tbl.length > 0 ) {
