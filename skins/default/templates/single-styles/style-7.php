...
if ( apply_filters( 'carz_filter_single_post_header', is_singular( 'post' ) || is_singular( 'attachment' ) ) ) {
	...
	ob_end_clean();
	$carz_with_featured_image = carz_is_with_featured_image( $carz_post_header /*, array( 'with_gallery' )*/);
	// Post title and meta
	ob_start();
	...
	if ( strpos( $carz_post_header, 'post_featured' ) !== false
		...
		<div class="post_header_wrap post_header_wrap_in_header post_header_wrap_style_<?php
            ...
            if ( $carz_with_featured_image ) {
                echo ' with_featured_image' . ( false && carz_get_theme_option( 'single_parallax' ) == 0 ? ' carz-full-height' : '' );
            }
		?>">
			<?php
