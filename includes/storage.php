...
if ( ! function_exists( 'carz_storage_get' ) ) {
	/**
	 ...
	 */
	function carz_storage_get( $var_name, $default = '' ) {
		global $CARZ_STORAGE;
		return isset( $CARZ_STORAGE[ $var_name ] ) ? $CARZ_STORAGE[ $var_name ] : $default;
	}
}

if ( ! function_exists( 'carz_storage_set' ) ) {
	/**
	 ...
	 */
	function carz_storage_set( $var_name, $value ) {
		global $CARZ_STORAGE;
		$CARZ_STORAGE[ $var_name ] = $value;
	}
}

if ( ! function_exists( 'carz_storage_empty' ) ) {
	/**
	 ...
	 */
	function carz_storage_empty( $var_name, $key = '', $key2 = '' ) {
		global $CARZ_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			return empty( $CARZ_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			return empty( $CARZ_STORAGE[ $var_name ][ $key ] );
		} else {
			return empty( $CARZ_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'carz_storage_isset' ) ) {
	/**
	 ...
	 */
	function carz_storage_isset( $var_name, $key = '', $key2 = '' ) {
		global $CARZ_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			return isset( $CARZ_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			return isset( $CARZ_STORAGE[ $var_name ][ $key ] );
		} else {
			return isset( $CARZ_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'carz_storage_unset' ) ) {
	/**
	 ...
	 */
	function carz_storage_unset( $var_name, $key = '', $key2 = '' ) {
		global $CARZ_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			if ( isset( $CARZ_STORAGE[ $var_name ][ $key ][ $key2 ] ) ) {
				unset( $CARZ_STORAGE[ $var_name ][ $key ][ $key2 ] );
			}
		} elseif ( '' !== $key ) {
			if ( isset( $CARZ_STORAGE[ $var_name ][ $key ] ) ) {
				unset( $CARZ_STORAGE[ $var_name ][ $key ] );
			}
		} else {
			if ( isset( $CARZ_STORAGE[ $var_name ] ) ) {
				unset( $CARZ_STORAGE[ $var_name ] );
			}
		}
	}
}

if ( ! function_exists( 'carz_storage_inc' ) ) {
	/**
	 ...
	 */
	function carz_storage_inc( $var_name, $value = 1 ) {
		global $CARZ_STORAGE;
		if ( empty( $CARZ_STORAGE[ $var_name ] ) ) {
			$CARZ_STORAGE[ $var_name ] = 0;
		}
		$CARZ_STORAGE[ $var_name ] += $value;
	}
}

if ( ! function_exists( 'carz_storage_concat' ) ) {
	/**
	 ...
	 */
	function carz_storage_concat( $var_name, $value ) {
		global $CARZ_STORAGE;
		if ( empty( $CARZ_STORAGE[ $var_name ] ) ) {
...
if ( ! function_exists( 'carz_storage_get_array' ) ) {
	/**
	 ...
	 */
	function carz_storage_get_array( $var_name, $key, $key2 = '', $default = '' ) {
		global $CARZ_STORAGE;
		if ( '' === $key2 ) {
			return ! empty( $var_name ) && '' !== $key && isset( $CARZ_STORAGE[ $var_name ][ $key ] ) ? $CARZ_STORAGE[ $var_name ][ $key ] : $default;
		} else {
			return ! empty( $var_name ) && '' !== $key && isset( $CARZ_STORAGE[ $var_name ][ $key ][ $key2 ] ) ? $CARZ_STORAGE[ $var_name ][ $key ][ $key2 ] : $default;
		}
	}
}

if ( ! function_exists( 'carz_storage_set_array' ) ) {
	/**
	 ...
	 */
	function carz_storage_set_array( $var_name, $key, $value ) {
		global $CARZ_STORAGE;
		if ( ! isset( $CARZ_STORAGE[ $var_name ] ) ) {
			$CARZ_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $CARZ_STORAGE[ $var_name ] ) ) {
			if ( '' === $key ) {
				$CARZ_STORAGE[ $var_name ][] = $value;
			} else {
...
if ( ! function_exists( 'carz_storage_set_array2' ) ) {
	/**
	 ...
	 * @param mixed  $value     A new value for the specified element of the array.
	 */
	function carz_storage_set_array2( $var_name, $key, $key2, $value ) {
		global $CARZ_STORAGE;
		if ( ! isset( $CARZ_STORAGE[ $var_name ] ) ) {
			$CARZ_STORAGE[ $var_name ] = array();
		}
		if ( ! isset( $CARZ_STORAGE[ $var_name ][ $key ] ) ) {
			$CARZ_STORAGE[ $var_name ][ $key ] = array();
		}
		if ( is_array( $CARZ_STORAGE[ $var_name ][ $key ] ) ) {
			if ( '' === $key2 ) {
				$CARZ_STORAGE[ $var_name ][ $key ][] = $value;
			} else {
				$CARZ_STORAGE[ $var_name ][ $key ][ $key2 ] = $value;
			}
		}
	}
}

if ( ! function_exists( 'carz_storage_merge_array' ) ) {
	/**
	 ...
	 */
	function carz_storage_merge_array( $var_name, $key, $value ) {
		global $CARZ_STORAGE;
		if ( ! isset( $CARZ_STORAGE[ $var_name ] ) ) {
			$CARZ_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			$CARZ_STORAGE[ $var_name ] = array_merge( $CARZ_STORAGE[ $var_name ], $value );
		} else {
			$CARZ_STORAGE[ $var_name ][ $key ] = array_merge( $CARZ_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'carz_storage_set_array_after' ) ) {
	/**
	 ...
	 */
	function carz_storage_set_array_after( $var_name, $after, $key, $value = '' ) {
		global $CARZ_STORAGE;
		if ( ! isset( $CARZ_STORAGE[ $var_name ] ) ) {
			$CARZ_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			carz_array_insert_after( $CARZ_STORAGE[ $var_name ], $after, $key );
		} else {
			carz_array_insert_after( $CARZ_STORAGE[ $var_name ], $after, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'carz_storage_set_array_before' ) ) {
	/**
	 * Insert new elements to the array in the global theme storage
	 ...
	 * @param mixed        $value     A new value to insert (if the argument $key is a string).
	 */
	function carz_storage_set_array_before( $var_name, $before, $key, $value = '' ) {
		global $CARZ_STORAGE;
		if ( ! isset( $CARZ_STORAGE[ $var_name ] ) ) {
			$CARZ_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			carz_array_insert_before( $CARZ_STORAGE[ $var_name ], $before, $key );
		} else {
			carz_array_insert_before( $CARZ_STORAGE[ $var_name ], $before, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'carz_storage_push_array' ) ) {
	/**
	 ...
	 */
	function carz_storage_push_array( $var_name, $key, $value ) {
		global $CARZ_STORAGE;
		if ( ! isset( $CARZ_STORAGE[ $var_name ] ) ) {
			$CARZ_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			array_push( $CARZ_STORAGE[ $var_name ], $value );
		} else {
			if ( ! isset( $CARZ_STORAGE[ $var_name ][ $key ] ) ) {
				$CARZ_STORAGE[ $var_name ][ $key ] = array();
			}
			array_push( $CARZ_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'carz_storage_pop_array' ) ) {
	/**
	 ...
	 */
	function carz_storage_pop_array( $var_name, $key = '', $defa = '' ) {
		global $CARZ_STORAGE;
		$rez = $defa;
		if ( '' === $key ) {
			if ( isset( $CARZ_STORAGE[ $var_name ] ) && is_array( $CARZ_STORAGE[ $var_name ] ) && count( $CARZ_STORAGE[ $var_name ] ) > 0 ) {
				$rez = array_pop( $CARZ_STORAGE[ $var_name ] );
			}
		} else {
			if ( isset( $CARZ_STORAGE[ $var_name ][ $key ] ) && is_array( $CARZ_STORAGE[ $var_name ][ $key ] ) && count( $CARZ_STORAGE[ $var_name ][ $key ] ) > 0 ) {
				$rez = array_pop( $CARZ_STORAGE[ $var_name ][ $key ] );
			}
		}
		return $rez;
	}
...
if ( ! function_exists( 'carz_storage_inc_array' ) ) {
	/**
	 ...
	 * @param int          $value     Optional. A value for increment. Default is 1. Specify -1 to decrement or any legal number value.
	 */
	function carz_storage_inc_array( $var_name, $key, $value = 1 ) {
		global $CARZ_STORAGE;
		if ( ! isset( $CARZ_STORAGE[ $var_name ] ) ) {
			$CARZ_STORAGE[ $var_name ] = array();
		}
		if ( empty( $CARZ_STORAGE[ $var_name ][ $key ] ) ) {
			$CARZ_STORAGE[ $var_name ][ $key ] = 0;
		}
		$CARZ_STORAGE[ $var_name ][ $key ] += $value;
	}
}

if ( ! function_exists( 'carz_storage_concat_array' ) ) {
	/**
	 ...
	 * @param int          $value     A value to concatenate.
	 */
	function carz_storage_concat_array( $var_name, $key, $value ) {
		global $CARZ_STORAGE;
		if ( ! isset( $CARZ_STORAGE[ $var_name ] ) ) {
			$CARZ_STORAGE[ $var_name ] = array();
		}
		if ( empty( $CARZ_STORAGE[ $var_name ][ $key ] ) ) {
			$CARZ_STORAGE[ $var_name ][ $key ] = '';
		}
		$CARZ_STORAGE[ $var_name ][ $key ] .= $value;
	}
}

if ( ! function_exists( 'carz_storage_call_obj_method' ) ) {
	/**
	 ...
	 */
	function carz_storage_call_obj_method( $var_name, $method, $param = null ) {
		global $CARZ_STORAGE;
		if ( null === $param ) {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $CARZ_STORAGE[ $var_name ] ) ? $CARZ_STORAGE[ $var_name ]->$method() : '';
		} else {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $CARZ_STORAGE[ $var_name ] ) ? $CARZ_STORAGE[ $var_name ]->$method( $param ) : '';
		}
	}
}

if ( ! function_exists( 'carz_storage_get_obj_property' ) ) {
	/**
	 ...
	 */
	function carz_storage_get_obj_property( $var_name, $prop, $default = '' ) {
		global $CARZ_STORAGE;
		return ! empty( $var_name ) && ! empty( $prop ) && isset( $CARZ_STORAGE[ $var_name ]->$prop ) ? $CARZ_STORAGE[ $var_name ]->$prop : $default;
	}
}
