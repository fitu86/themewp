...
if ( ! defined( 'CARZ_THEME_FREE' ) ) {
	define( 'CARZ_THEME_FREE', false );
}
if ( ! defined( 'CARZ_THEME_FREE_WP' ) ) {
	define( 'CARZ_THEME_FREE_WP', false );
}

// If this theme support external updates
if ( ! defined( 'CARZ_THEME_ALLOW_UPDATE' ) ) {
	define( 'CARZ_THEME_ALLOW_UPDATE', true );
}

// If this theme uses skins
if ( ! defined( 'CARZ_ALLOW_SKINS' ) ) {
	define( 'CARZ_ALLOW_SKINS', true );
}
if ( ! defined( 'CARZ_DEFAULT_SKIN' ) ) {
	define( 'CARZ_DEFAULT_SKIN', 'default' );
}
if ( ! defined( 'CARZ_REMEMBER_SKIN' ) ) {
	define( 'CARZ_REMEMBER_SKIN', false );
}
...
if ( ! function_exists( 'carz_get_theme_pro_key' ) ) {
	/**
	 ...
	 */
	function carz_get_theme_pro_key() {
		$theme_pro_key = get_option( sprintf( 'purchase_code_src_%s', get_template() ) );
		if ( empty( $theme_pro_key ) ) {
			$theme_pro_key = carz_storage_get( 'theme_pro_key' );
		}
		return $theme_pro_key;
	}
}

if ( ! function_exists( 'carz_trx_updater_get_theme_info' ) ) {
	add_filter( 'trx_updater_filter_get_theme_info', 'carz_trx_updater_get_theme_info' );
	/**
	 ...
	 */
	function carz_trx_updater_get_theme_info( $info ) {
		$engine = carz_storage_get( 'engine' );
...
if ( ! function_exists( 'carz_original_theme_slug' ) ) {
	add_filter( 'carz_filter_original_theme_slug', 'carz_original_theme_slug' );
	/**
...
if ( ! function_exists( 'carz_importer_set_options' ) ) {
	add_filter( 'trx_addons_filter_importer_options', 'carz_importer_set_options', 9 );
	/**
	 ...
	 */
	function carz_importer_set_options( $options = array() ) {
		if ( is_array( $options ) ) {
			...
			// Prepare demo data
			$options['demo_url'] = esc_url( carz_get_protocol() . ':' . carz_storage_get( 'theme_demofiles_url' ) );
			// Required plugins
			$options['required_plugins'] = array_keys( carz_storage_get( 'required_plugins' ) );
...
if ( ! function_exists( 'carz_ocdi_set_options' ) ) {
	add_filter( 'trx_addons_filter_ocdi_options', 'carz_ocdi_set_options', 9 );
	/**
	 ...
	 * @return array          Processed options with a theme-specific values.
	 */
	function carz_ocdi_set_options( $options = array() ) {
		if ( is_array( $options ) ) {
			// Prepare demo data
			$options['demo_url'] = esc_url( carz_get_protocol() . ':' . carz_storage_get( 'theme_demofiles_url' ) );
			// Required plugins
...
if ( ! function_exists( 'carz_customizer_theme_setup1' ) ) {
	add_action( 'after_setup_theme', 'carz_customizer_theme_setup1', 1 );
	/**
	 ...
	 */
	function carz_customizer_theme_setup1() {
		carz_storage_set(
			'settings', array(
				...
				'allow_fullscreen'          => false,                   // Allow cases 'fullscreen' and 'fullwide' (if a next settings is true) for the body style in the Theme Options
																		...
																		// (can be removed if filter 'carz_filter_allow_fullscreen' return false)
