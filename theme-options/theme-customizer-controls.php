...
class Carz_Customize_Theme_Control extends WP_Customize_Control {
	...
	protected function end_render_field() {
		if ( ! empty( $this->input_attrs['data-pro-only'] ) ) {
			carz_show_layout( carz_add_inherit_cover( $this->id, array( 'type' => $this->type, 'pro_only' => true ) ) );
		}
		?>
		</div>
		<?php
	...
	protected function render_field_description() {
		if ( ! empty( $this->description ) ) {
			?>
			<span class="customize-control-description description"><?php carz_show_layout( $this->description ); ?></span>
			<?php
		}
	}
...
/**
 * Class Carz_Customize_Info_Control.
 * 
 * A control to display an information block in the Customizer.
 */
class Carz_Customize_Info_Control extends Carz_Customize_Theme_Control {
	public $type = 'info';
...
/**
 * Class Carz_Customize_Hidden_Control.
 * 
 ...
 */
class Carz_Customize_Hidden_Control extends Carz_Customize_Theme_Control {
	public $type = 'hidden';
	...
	protected function render_content() {
		...
		if ( 'last_option' == $this->id && carz_storage_get( 'need_footer_scripts', false ) ) {
			carz_storage_set( 'need_footer_scripts', false );
...
/**
 * Class Carz_Customize_Button_Control.
 * 
 ...
 */
class Carz_Customize_Button_Control extends Carz_Customize_Theme_Control {
	public $type = 'button';
	...
	protected function render_content() {
		...
		if ( ! empty( $this->input_attrs['link'] ) ) {
			...
			<a href="<?php echo esc_url( $this->input_attrs['link'] ); ?>"<?php
				echo carz_external_links_target( true );
...
class Carz_Customize_Switch_Control extends Carz_Customize_Theme_Control {
	...
	protected function render_content() {
		...
		$this->render_field_description();
		?>
		<label class="customize-control-field-wrap carz_options_item_switch">
			<input type="hidden"
				<?php
				$this->link();
				if ( ! empty( $this->input_attrs['var_name'] ) ) {
...
/**
 * Class Carz_Customize_Icon_Control.
 * 
 ...
 */
class Carz_Customize_Icon_Control extends Carz_Customize_Theme_Control {
	public $type = 'icon';
...
/**
 * Class Carz_Customize_Choice_Control.
 * 
 ...
 */
class Carz_Customize_Choice_Control extends Carz_Customize_Theme_Control {
	public $type = 'choice';
...
/**
 * Class Carz_Customize_Scheme_Editor_Control.
 * 
 * A control to render a scheme editor in the Customizer.
 * This control allows users to edit each color scheme from the skin.
 */
class Carz_Customize_Scheme_Editor_Control extends Carz_Customize_Theme_Control {
		...
		/>
			...
			carz_show_layout(
				carz_show_custom_field(
					...
					$this->input_attrs,
					carz_unserialize( $this->input_attrs['value'], true )
				)
			);
			?>
		</span>
...
/**
 * Class Carz_Customize_Text_Editor_Control.
 * 
 * A control to render a text editor in the Customizer.
 * This control allows users to input rich text content.
 */
class Carz_Customize_Text_Editor_Control extends Carz_Customize_Theme_Control {
	...
	protected function render_content() {
		...
		carz_storage_set( 'need_footer_scripts', true );
	}
}


/**
 * Class Carz_Customize_Range_Control.
 * 
 ...
 */
class Carz_Customize_Range_Control extends Carz_Customize_Theme_Control {
	public $type = 'range';
	...
	protected function render_content() {
		...
		<span class="customize-control-field-wrap"><input type="<?php echo ! $show_value ? 'hidden' : 'text'; ?>" 
			...
			$this->link();
			if ( $show_value ) {
				echo ' class="carz_range_slider_value"';
			}
			if ( ! empty( $this->input_attrs['var_name'] ) ) {
				echo ' data-var_name="' . esc_attr( $this->input_attrs['var_name'] ) . '"';
			}
