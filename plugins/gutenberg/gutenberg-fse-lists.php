...
if ( ! function_exists( 'carz_gutenberg_fse_list_header_styles' ) ) {
	add_filter( 'carz_filter_list_header_styles', 'carz_gutenberg_fse_list_header_styles');
	/**
	 * Add a WordPress FSE templates to the headers list.
	 * 
	 * @hooked 'carz_filter_list_header_styles'
	 * 
	 * @param array $list  Optional. An array with a list of headers from other available Page Builders.
	 */
	function carz_gutenberg_fse_list_header_styles( $list = array() ) {
		...
		$layouts = carz_get_list_posts(
			false, array(
				'post_type'    => CARZ_FSE_TEMPLATE_PART_PT,
				'orderby'      => 'ID',
				'order'        => 'asc',
				'not_selected' => false,
				'tax_query'    => array(
										...
										array(
											...
											'field'    => 'name',
											'terms'    => CARZ_FSE_TEMPLATE_PART_AREA_HEADER,
										)
									)
			)
		);
		...
		if ( ! empty( $data['templateParts'] ) && is_array( $data['templateParts'] ) ) {
			foreach ( $data['templateParts'] as $template ) {
				if ( ! empty( $template['area'] )
					&& CARZ_FSE_TEMPLATE_PART_AREA_HEADER == $template['area']
...
if ( ! function_exists( 'carz_gutenberg_fse_list_footer_styles' ) ) {
	add_filter( 'carz_filter_list_footer_styles', 'carz_gutenberg_fse_list_footer_styles');
	/**
	 * Add a WordPress FSE templates to the footers list.
	 * 
	 * @hooked 'carz_filter_list_footer_styles'
	 * 
	 * @param array $list  Optional. An array with a list of footers from other available Page Builders.
	 */
	function carz_gutenberg_fse_list_footer_styles( $list = array() ) {
		if ( ! carz_gutenberg_is_fse_theme() ) {
			return $list;
		}
		$new_list = array();
			...
			false, array(
				...
				'tax_query'    => array(
										...
										array(
											'taxonomy' => 'wp_template_part_area',
											'field'    => 'name',
											'terms'    => CARZ_FSE_TEMPLATE_PART_AREA_FOOTER,
										)
									)
			)
		);
		...
		if ( ! empty( $data['templateParts'] ) && is_array( $data['templateParts'] ) ) {
			foreach ( $data['templateParts'] as $template ) {
				if ( ! empty( $template['area'] )
					&& CARZ_FSE_TEMPLATE_PART_AREA_FOOTER == $template['area']
					&& ! in_array( $template['title'], $new_list )
				) {
...
if ( ! function_exists( 'carz_gutenberg_fse_list_sidebar_styles' ) ) {
	add_filter( 'carz_filter_list_sidebar_styles', 'carz_gutenberg_fse_list_sidebar_styles');
	/**
	 * Add a WordPress FSE templates to the sidebars list.
	 * 
	 * @hooked 'carz_filter_list_sidebar_styles'
	 * 
	 * @param array $list  Optional. An array with a list of sidebars from other available Page Builders.
	 */
	function carz_gutenberg_fse_list_sidebar_styles( $list = array() ) {
		if ( ! carz_gutenberg_is_fse_theme() ) {
			return $list;
		}
		$new_list = array();
			...
			false, array(
				...
				'tax_query'    => array(
										...
										array(
											'taxonomy' => 'wp_template_part_area',
											'field'    => 'name',
											'terms'    => CARZ_FSE_TEMPLATE_PART_AREA_SIDEBAR,
										)
									)
			)
		);
		...
		if ( ! empty( $data['templateParts'] ) && is_array( $data['templateParts'] ) ) {
			foreach ( $data['templateParts'] as $template ) {
				if ( ! empty( $template['area'] )
					&& CARZ_FSE_TEMPLATE_PART_AREA_SIDEBAR == $template['area']
					&& ! in_array( $template['title'], $new_list )
				) {
