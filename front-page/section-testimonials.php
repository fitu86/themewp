<div class="front_page_section front_page_section_testimonials<?php
	$carz_scheme = carz_get_theme_option( 'front_page_testimonials_scheme' );
	if ( ! empty( $carz_scheme ) && ! carz_is_inherit( $carz_scheme ) ) {
		echo ' scheme_' . esc_attr( $carz_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( carz_get_theme_option( 'front_page_testimonials_paddings' ) );
	if ( carz_get_theme_option( 'front_page_testimonials_stack' ) ) {
		echo ' sc_stack_section_on';
	}
?>"
		<?php
		$carz_css      = '';
		$carz_bg_image = carz_get_theme_option( 'front_page_testimonials_bg_image' );
...
<?php
	// Add anchor
	$carz_anchor_icon = carz_get_theme_option( 'front_page_testimonials_anchor_icon' );
	$carz_anchor_text = carz_get_theme_option( 'front_page_testimonials_anchor_text' );
...
?>
	...
	<?php
	if ( carz_get_theme_option( 'front_page_testimonials_fullheight' ) ) {
	...
	?>
			...
			<?php
			$carz_css      = '';
			$carz_bg_mask  = carz_get_theme_option( 'front_page_testimonials_bg_mask' );
			$carz_bg_color_type = carz_get_theme_option( 'front_page_testimonials_bg_color_type' );
			if ( 'custom' == $carz_bg_color_type ) {
				$carz_bg_color = carz_get_theme_option( 'front_page_testimonials_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_testimonials_content_wrap content_wrap">
			<?php
			// Caption
			$carz_caption = carz_get_theme_option( 'front_page_testimonials_caption' );
			if ( ! empty( $carz_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_testimonials_caption front_page_block_<?php echo ! empty( $carz_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $carz_caption, 'carz_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$carz_description = carz_get_theme_option( 'front_page_testimonials_description' );
			if ( ! empty( $carz_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_testimonials_description front_page_block_<?php echo ! empty( $carz_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $carz_description ), 'carz_kses_content' ); ?></div>
				<?php
			}
			...
			<div class="front_page_section_output front_page_section_testimonials_output">
				...
				if ( is_active_sidebar( 'front_page_testimonials_widgets' ) ) {
					...
					if ( ! carz_exists_trx_addons() ) {
						...
						carz_customizer_need_widgets_message( 'front_page_testimonials_caption', 'ThemeREX Addons - Testimonials' );
					}
				}
				?>
			</div>
