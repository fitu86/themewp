...
jQuery( window ).on( 'load', function() {
	...
	if ( $editor_wrapper.length ) {
		...
		if ( typeof window.MutationObserver !== 'undefined' ) {
			...
			carz_create_observer( 'carz-check-visual-editor-wrapper', $editor_wrapper, function( mutationsList ) {
				carz_gutenberg_editor_init();
			} );
		} else {
		 	carz_gutenberg_editor_init();
		}
	...
	function carz_gutenberg_editor_object() {
		var editor = {
			$editor: false,
			$frame: false,
			...
			if ( $editor.length ) {
				...
				if ( editor.$frame.length ) {
					if ( editor.$frame.get(0).contentDocument ) {
						...
						if ( editor.$styles_wrapper.hasClass( 'carz_inited' ) ) {
					...
					if ( ! editor.$editor.hasClass( 'carz_inited' ) ) {
	...
	// Init on page load
	function carz_gutenberg_editor_init() {

		// Get Gutenberg editor object
		var editor = carz_gutenberg_editor_object();
		if ( ! editor.$editor ) {
			return;
		}
		...
		function add_class_with_color_scheme( $obj ) {
			if ( ! $obj.hasClass( 'scheme_' + CARZ_STORAGE['color_scheme'] ) ) {
				$obj.addClass( 'scheme_' + CARZ_STORAGE['color_scheme'] );
			}
		}

		function add_class_with_overridden_options( $obj ) {
			for ( var i in CARZ_STORAGE['override_classes'] ) {
				$obj.addClass( CARZ_STORAGE['override_classes'][i].replace( '%s', CARZ_STORAGE[ i ] ) );
			}
		}
		...
		if ( editor_type == 'post' && editor.$styles_wrapper ) {
			...
			editor.$styles_wrapper.addClass( carz_get_class_by_prefix( editor.$editor.attr( 'class' ), 'post-type-' ) );
			...
			if ( editor.$writing_flow && CARZ_STORAGE['sidebar_position'] != 'hide' ) {
				if ( carz_apply_filters( 'carz_filter_wrap_gutenberg_editor_with_sidebar', false ) ) {
		...
		if ( editor_type == 'site' && editor.$styles_wrapper ) {
			...
			add_class_with_overridden_options( editor.$styles_wrapper );
			// Mark as inited
			editor.$styles_wrapper.addClass( 'carz_inited' );
		}

		// Widgets Editor
		//----------------------------------------------------------
		if ( editor_type == 'widgets' && editor.$writing_flow ) {
			if ( carz_apply_filters( 'carz_filter_add_scheme_to_widgets_editor', true ) ) {
				// Create an observer to add the class 'scheme_xxx' to the each new sidebar
				carz_remove_observer( 'carz-check-editor-styles-wrapper' );
				carz_create_observer( 'carz-check-editor-styles-wrapper', editor.$writing_flow, function( mutationsList ) {
		...
		editor.$editor.addClass( 'carz_inited' );
	}
} );
