...
jQuery( document ).ready(
	function() {
		...
		carz_add_filter('trx_addons_filter_sc_classes', function(classes, settings) {
		...
		setTimeout( function() {
			if ( window.elementor !== undefined ) {
				...
				jQuery( '#elementor-panel' )
					.on( 'input change', '[data-setting^="carz_options_"]', function (e) {
						...
						var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.carz_post_editor' );
						if ( link.length > 0 ) {
							carz_change_post_edit_link_elementor( link );
						}
					} )
					.on( 'click', '.carz_post_editor', function(e) {
						carz_change_post_edit_link_elementor( jQuery(this) );
						if (jQuery(this).hasClass('carz_hidden' )) {
							e.preventDefault();
							return false;
						}
					});
		...
		function carz_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
