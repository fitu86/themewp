...
if ( function_exists( 'carz_elementor_is_preview' ) && carz_elementor_is_preview() ) {
	...
	if ( have_posts() ) {
		...
		if ( isset( $GLOBALS['post'] ) && is_object( $GLOBALS['post'] ) ) {
			carz_storage_set( 'blog_archive_template_post', $GLOBALS['post'] );
		}
	}
	...
	carz_new_main_query(
		array(
			'post_type'      => carz_get_theme_option( 'post_type' ),
			'category'       => carz_get_theme_option( 'parent_cat' ),
	...
	get_template_part( apply_filters( 'carz_filter_get_template_part', carz_blog_archive_get_template() ) );
}
