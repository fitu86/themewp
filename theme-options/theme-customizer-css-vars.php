<?php
if ( ! function_exists( 'carz_add_css_vars' ) ) {
	add_filter( 'carz_filter_get_css', 'carz_add_css_vars', 1, 2 );
	/**
	 * Add a theme-specific fonts, vars and colors to the custom CSS
	 * 
	 * @hooked carz_filter_get_css, 1
	 * 
	 ...
	 */
	function carz_add_css_vars( $css, $args ) {
						...
						if ( is_array( $font ) ) {
							foreach ( $font as $css_prop => $css_value ) {
								...
								if ( empty( $suffix ) || ! empty( $font["{$css_prop}{$suffix}"] ) ) {
									...
									if ( ( ! empty( $font["{$param}{$suffix}"] ) && ! carz_is_inherit( $font["{$param}{$suffix}"] ) ) || strpos( $css_prop, 'color' ) === false ) {
										$tmp .= "--theme-font-{$tag}_{$css_name}: " . ( ! empty( $font["{$param}{$suffix}"] ) && ! carz_is_inherit( $font["{$param}{$suffix}"] )
																		? ( in_array( $css_prop, array( 'font-size', 'letter-spacing', 'margin-top', 'margin-bottom', 'border-width', 'border-radius' ) )
																			? carz_prepare_css_value( $font["{$param}{$suffix}"] )
																			: $font["{$param}{$suffix}"]
																			)
																		: 'inherit'
																	) . ";\n";
		...
		if ( isset( $css['vars'] ) && isset( $args['vars'] ) ) {
			global $CARZ_STORAGE;
			if ( is_array( $CARZ_STORAGE['options'] ) && count( $CARZ_STORAGE['options'] ) > 0 ) {
				$breakpoints = carz_get_theme_breakpoints();
				foreach ( $breakpoints as $bp => $bpv ) {
				...
				foreach ( $CARZ_STORAGE['options'] as $option => $value ) {
					if ( ! empty( $value['css'] ) ) {
						...
						foreach ( $breakpoints as $bp => $bpv ) {
							...
							if ( $css_value !== '' ) {
								$css_value = carz_prepare_css_value( $css_value );
		...
		if ( isset( $css['vars'] ) && isset( $args['vars'] ) ) {
			...
			if ( is_array( $vars ) && count( $vars ) > 0 ) {
				...
				if ( isset( $vars['sidebar_gap'] ) ) {
					$vars['sidebar_gap_width'] = carz_prepare_css_value( $vars['sidebar_gap'] );
				}
				// Remove calculated values from css variables
				$exclude = apply_filters( 'carz_filter_exclude_theme_vars', array( 'sidebar_gap' ) );	//Old case: array( 'sidebar_width', 'sidebar_gap' )
